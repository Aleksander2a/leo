...
export function RunDetailTabs({
  ...
  const contextEntries = timeline.filter(
    (entry) => entry.kind === "context_built",
  );
  const toolEntries = timeline.filter(
    (entry) =>
      entry.kind === "tool_call" || entry.kind === "evidence_normalized",
  );
  const verificationEntries = timeline.filter(
    (entry) => entry.kind === "verification",
  );
  const modelCallEntries = timeline.filter(
    (entry) => entry.kind === "model_called",
  );
...
const PLAN_STATUS_STYLE: Record<string, string> = {
  satisfied: "text-emerald-600 dark:text-emerald-400",
  pending: "text-amber-600 dark:text-amber-400",
  abandoned: "text-gray-500 dark:text-gray-400",
};

function StepPlan({ plan }: { plan: RunReasoning["plan"] }) {
  if (plan.length === 0) {
    return null;
  }
  return (
    <div className="mb-4 rounded-md border border-gray-200 p-3 dark:border-gray-800">
      ...
      <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
        The run could not finish while a step was pending. A step naming a tool
        is only satisfied by a real result from that tool.
      </p>
      <ul className="mt-3 space-y-2 text-sm">
        {plan.map((step) => (
          <li key={step.key} className="flex flex-wrap items-baseline gap-x-2">
            <span
              className={`font-mono text-xs ${PLAN_STATUS_STYLE[step.status] ?? ""}`}
            >
              {step.status}
            </span>
  ...
  if (steps.length === 0 && plan.length === 0) {
    return (
      <p className="text-sm text-gray-500 dark:text-gray-400">
        No reasoning trace was recorded for this run. Runs that answer in a
        single turn without calling a tool may finish before a step is written.
      </p>
    );
  }
  return (
...
function ContextTab({ entries }: { entries: TimelineEntry[] }) {
  if (entries.length === 0) {
    return (
      <p className="text-sm text-gray-400">
        No context-assembly events recorded for this run.
      </p>
    );
  }
  return (
    <div className="space-y-4">
      {entries.map((entry) => {
        const manifest = entry.raw_payload.source_manifest as
            ...
            {manifest ? (
              <div className="mb-3 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
                <Metric
                  ...
                  value={manifest.included_estimated_tokens}
                />
                <Metric
                  label="Excluded tokens"
                  value={manifest.excluded_estimated_tokens}
                />
                <Metric
                  label="Included sources"
                  value={(manifest.included_source_ids as unknown[])?.length}
                />
                <Metric
                  label="Excluded sources"
                  value={(manifest.excluded_source_ids as unknown[])?.length}
                />
              </div>
            ) : null}
            ...
            {detail.observations.map((observation) => (
              ...
              >
                <div className="mb-2 flex flex-wrap items-center gap-2 text-xs">
                  ...
                  <ProvenanceBadge
                    callKind={observation.call_kind}
                ...
                {observation.rejection_code ? (
                  <p className="mb-2 text-xs text-red-500">
                    rejected: {observation.rejection_code}
                  </p>
                ) : null}
...
function ClaimsTab({
  ...
  detail: RunDetail;
  verificationEntries: TimelineEntry[];
}) {
  return (
    <div className="space-y-6">
      <section>
...
function DeliveryTab({ detail }: { detail: RunDetail }) {
  if (detail.deliveries.length === 0) {
    return (
      <p className="text-sm text-gray-400">
        No Slack delivery attempts were recorded for this run.
      </p>
    );
  }
  return (
      ...
      {detail.deliveries.map((delivery) => (
        ...
        >
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="font-mono text-xs text-gray-400">
              {delivery.destination_channel_id}
            </span>
          ...
          <p className="mt-1 text-xs text-gray-400">
            ...
            {delivery.receipt_message_ts
              ? ` · receipt ${delivery.receipt_message_ts}`
              : ""}
          </p>
          {delivery.last_error ? (
            <p className="mt-1 text-xs text-red-500">{delivery.last_error}</p>
          ) : null}
        </li>
      ))}
    </ul>
